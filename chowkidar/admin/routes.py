...
@admin_view.route('/admin/audits/<string:user_email>/<string:audit_name>', methods=['GET', 'POST'])
@login_required
...
def admin_scan_result(user_email, audit_name):
    ...
    if audit:
        ...
        if not output:
            ...
            return redirect(url_for('admin_vulnerabilities', user_email=audit.Auditor.email, audit_name=audit_name))
    else:
        flash('Unfortunately, you do not have the privilege to access this audit', 'danger')
        return redirect(url_for('admin_view.all_audits'))
